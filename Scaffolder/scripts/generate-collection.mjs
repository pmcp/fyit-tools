...
function parseArgs() {
  ...
  if (!layer || !collection) {
    console.log('Usage: node scripts/generate-collection.next.mjs <layer> <collection> [--fields-file <path>] [--dialect=pg|sqlite] [--auto-relations] [--dry-run] [--no-db] [--force]')
    process.exit(1)
  }
  
  return { layer, collection, fieldsFile, dialect, autoRelations, dryRun, noDb, force }
}
...
async function updateLayerRootConfig(layer, collectionName) {
  const cases = toCase(collectionName)
...
`
    ...
    if (extendsMatch) {
      ...
      if (!currentExtends.includes(newCollection)) {
        ...
        console.log(`✓ ${layer} layer root config already extends ${cases.plural}`)
      }
    }
  } catch (error) {
...
async function writeScaffold({ layer, collection, fields, dialect, autoRelations, dryRun, noDb, force = false, config = null }) {
  ...
  // Update layer root nuxt.config.ts to extend the new collection
  await updateLayerRootConfig(layer, collection)
