...
import type { Test2FormProps, Test2FormData } from '../../types'
import { z } from 'zod'

const { send } = useCrud()

const props = defineProps<Test2FormProps>()

const { defaultValue, schema } = useTest2s()

// Create a reactive form state with proper typing
const state = reactive<Test2FormData & { id?: string | null }>({
  id: null,
  name: '',
  description: ''
})
