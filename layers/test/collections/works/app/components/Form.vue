...
import type { WorkFormProps, WorkFormData } from '../../types'
import { z } from 'zod'

const { send } = useCrud()

const props = defineProps<WorkFormProps>()

const { defaultValue, schema } = useWorks()

// Create a reactive form state with proper typing
const state = reactive<WorkFormData & { id?: string | null }>({
  id: null,
  name: '',
  description: ''
})
