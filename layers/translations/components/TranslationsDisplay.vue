          ...
          <div class="p-3 max-w-sm">
            <div class="flex items-center justify-between mb-2">
              <span class="text-xs font-medium text-gray-500 dark:text-gray-400">
                {{ lang.toUpperCase() }}
                <span v-if="isUsingFallback(lang)" class="text-orange-500 ml-1">(fallback to EN)</span>
              </span>
              <UButton
                icon="i-lucide-copy"
                size="xs"
            ...
            <p class="text-sm break-words">{{ getTranslationForLang(lang) || 'No translation' }}</p>
        ...
        <div class="space-y-4">
          <div v-if="selectedLang && isUsingFallback(selectedLang)" class="p-2 bg-orange-50 dark:bg-orange-900/20 rounded-md">
            <p class="text-sm text-orange-600 dark:text-orange-400 flex items-center gap-2">
              ...
              Using English translation as fallback
            </p>
          </div>
          <div class="bg-gray-100 dark:bg-gray-800 rounded-lg p-4">
            <div class="flex items-center justify-between mb-3">
              ...
              <UButton
                ...
                color="neutral"
                variant="subtle"
                @click="copyToClipboard(getTranslationForLang(selectedLang))"
              />
            </div>
            <p class="text-sm break-words whitespace-pre-wrap">
              {{ getTranslationForLang(selectedLang) || 'No translation available' }}
            </p>
          </div>
