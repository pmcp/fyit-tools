...
export function useT() {
   ...
   * @returns Translated string or VNode (in dev mode)
   */
  const translate = (key: string, options: TranslationOptions = {}): string | VNode => {
    ...
    // In dev mode with dev mode enabled, wrap with DevTranslationWrapper
    if (isDev && devModeEnabled.value) {
      const DevTranslationWrapper = resolveComponent('TranslationsDevTranslationWrapper')
