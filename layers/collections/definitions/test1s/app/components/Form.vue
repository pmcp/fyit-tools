...
import type { Test1FormProps, Test1FormData } from '../../types'
import { z } from 'zod'

const { send } = useCrud()

const props = defineProps<Test1FormProps>()

const { defaultValue, schema } = useTest1s()

// Create a reactive form state with proper typing
const state = reactive<Test1FormData & { id?: string | null }>({
  id: null,
  name: '',
  description: ''
})
